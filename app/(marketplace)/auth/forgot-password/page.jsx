...
export default function ForgotPasswordPage() {
  ...
  async function handleSendOtp(e) {
    ...
    setSendingOtp(true);
    try {
      const res = await fetch("/api/v1/auth/forgot-password/send", {
        method: "POST",
  ...
  return (
    <div className="max-w-sm mx-auto px-4 sm:px-6 py-16">
      <h1 className="font-display text-2xl text-gray-900 mb-1">Forgot Password</h1>
      ...
      {step === 1 ? (
        <form onSubmit={handleSendOtp} noValidate className="space-y-4">
          <div>
            <label htmlFor="identifier" className="block text-xs text-gray-500 mb-1.5">
        ...
        <form onSubmit={handleResetPassword} noValidate className="space-y-4">
          <div>
            <label htmlFor="otp" className="block text-xs text-gray-500 mb-1.5">
              Verification Code
            </label>
            <input
              id="otp"
          ...
          <button
            ...
            className="w-full text-sm text-gray-500 mt-2 disabled:opacity-60"
          >
            {resendCooldown > 0
