...
export default function SellerForgotPasswordPage() {
  ...
  async function handleSendOtp(e) {
    ...
    setSendingOtp(true);
    try {
      const res = await fetch("/api/v1/auth/seller/forgot-password/send", {
        method: "POST",
  ...
  async function handleResetPassword(e) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/v1/auth/seller/forgot-password/reset", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <div className="min-h-screen bg-white flex flex-col">
      <header className="border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          <Link href="/seller/login" className="flex items-center gap-2 text-blue-600 font-semibold text-sm">
            <ArrowLeft size={16} />
              ...
              <div>
                ...
                {notFound && (
                  <p className="text-sm text-gray-600 mt-2">
                    No seller account found with that phone number.{" "}
                    <Link href="/seller/login" className="text-blue-600 underline">
                      Back to sign in
                    </Link>
                  </p>
                )}
