...
export default function AgentDeliveryPage() {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-muted mb-8">Manage delivery options for your city.</p>
      {loading ? (
        <p className="text-sm text-muted">Loading...</p>
      ) : (
        <DeliveryMethodsList initialMethods={methods} towns={towns} currentActorName={session?.user?.name || "Agent"} />
      )}
    </div>
  );
}
